# 🐾 MeltPet — Evidence-Backed Pet Intelligence Hub

**Live site:** [meltpet.com](https://www.meltpet.com)

MeltPet is an independent, ad-free resource for pet owners who want real answers — not AI-generated filler, not sponsored picks, not recycled forum threads. Every article cites peer-reviewed research. Every cost estimate is receipt-tracked. No exceptions.

---

## What's Inside (~65 pages)

### 🔧 Interactive Tools (10)
Breed finders, cost calculators, food checkers, name generators — all client-side, no signup required.

### 🐶 Dog Breed Guides (9)
Golden Retriever · French Bulldog · German Shepherd · Labrador Retriever · Australian Shepherd · Dachshund · Poodle · Pembroke Welsh Corgi · Golden vs Labrador

### 🐱 Cat Breed Guides (5)
Maine Coon · Ragdoll · Bengal · Sphynx · British Shorthair

### 🧠 Behavior Explainers (16)
Why dogs howl · Why cats follow you to the bathroom · Why dogs stare · Why dogs eat grass · Midnight zoomies · Head tilts · Kneading · Tail wagging · Why cats sleep so much · Why cats love boxes · + more

### 🩺 Health & Training (14)
Separation anxiety protocol · Dental care · Leash training · Fireworks calm · Cat vomiting color guide · Kidney disease prevention · DIY vs pro grooming · + more

### 💰 Cost & Finance (8)
First-year costs · Hidden costs · Pet insurance worth it · Monthly dog cost · Dog food comparison · Cat litter comparison · + more

---

## 🛠 Tech Stack

- **Pure static HTML/CSS/JS** — no frameworks, no build step, no npm
- **Cloudflare Pages** — global edge deployment
- **nav.js** — single-source navigation engine (desktop dropdowns, mobile drawer, search index)
- **No analytics tracking** — no cookies, no popups, no consent banners
- **~65 hand-written pages** — zero AI-generated content

---

## 👩‍🔬 Editorial Standards

Lead researcher **Sarah Mitchell** spent five years in pet food R&D. Every health claim is cross-checked against veterinary literature. Sources are cited at the bottom of each guide. We don't cite studies we haven't read in full.

Read more: [Sarah Mitchell bio](https://www.meltpet.com/sarah-mitchell) · [About MeltPet](https://www.meltpet.com/about)

---

## 📜 License

Content © 2026 MeltPet. All rights reserved.

Code (HTML templates, CSS, JavaScript) is open for inspection and educational use. Attribution appreciated if you reuse patterns.

---

*As an Amazon Associate, MeltPet earns from qualifying purchases.*
